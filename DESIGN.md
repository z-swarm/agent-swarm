...
| **W36a** | ① `SecretRef` 协议 (literal / `${VAR}` / `secret://key`) 三种格式 ② `parse_secret_ref(ref)` 解析函数 + 单元测试 ③ `create_app(secret_manager: SecretManager \| None = None)` 关键字注入; 缺省自动实例化 `EnvSecretManager` ④ `JWTIssuer(JWTConfig(secret_ref, secret_manager))` 持 `SecretManager` + 内部 `(key, version) → bytes` cache ⑤ 轮换不重启: `SecretMetadata.version` 变化 → cache miss → 重读; cache TTL 默认 5 分钟 (与 W20 VaultSecretManager 一致) ⑥ 降级路径: `secret_manager.get` 失败时 cache 命中 → 继续用; cache miss → `JWTError` ⑦ CLI `--web-jwt-secret-ref` + `--web-secret-manager {env,vault}`; `--web-secret-manager vault` 需 `--vault-url` / `--vault-role-id` / `--vault-secret-id` ⑧ Golden Case G-026: 4 cases (Phase A token verify / Phase B rotate / Phase C 旧 token 仍 verify within TTL / Phase D 新 token verify) ⑨ `tools/verify_w36a_dod.py` 8 项全过 ⑩ ruff 0 + mypy 0 + 全量 0 新失败 (W35 baseline) |
| **W36b** | ① `POST /api/review` 路由接受 `pr_ref` (default `"main..HEAD"`) + 调 `run_simple_review` 同步返 `ReviewReport` ② `/review` 页面 + HTMX 表单 (PR ref 输入 + Run Review 按钮 + 结果展示 partial) ③ nav 增 /review 入口 ④ 鉴权: 写路径强制 Bearer token (W34 模式) ⑤ 错误处理: 无 git repo / 无 diff / 异常 → 友好 JSON 错误 ⑥ `tests/unit/test_web_review.py` ≥8 cases (no auth/no body/auth/简单 review/有 security 问题/无 git repo) ⑦ Golden Case G-027 端到端: 干净 PR 0 finding / 有问题 PR ≥1 finding (W13 7 类规则) ⑧ `tools/verify_w36b_dod.py` 8 项全过 ⑨ ruff 0 + mypy 0 + 全量 0 新失败 (W36a baseline) ⑩ CHANGELOG W36b 节点 + docs/MEMORY.md + docs/P5-RETRO.md |
| **W36c** | ① `parse_secret_ref` 识别 `vault://path#field` URI (闭环 W36a 留口子) ② `SecretRef` 扩展 `field: str \| None` 字段 ③ `JWTIssuer.resolve_secret` 处理 vault:// kind: `mgr.get(path)` + JSON 提取 `field` ④ `create_app` 自动实例化 `VaultSecretManager` (vault:// + 无 secret_manager 时) 需 `--vault-url/--vault-role-id/--vault-secret-id` ⑤ CLI `--web-jwt-secret-ref` 接受 `vault://` ⑥ `tests/unit/test_web_jwt_vault_ref.py` ≥6 cases (parse / field 提取 / JSON 解析 / 缺 field / 错误) ⑦ Golden Case G-028 端到端: vault:// 解析 + 轮换不重启 + JSON field 提取 ⑧ `tools/verify_w36c_dod.py` 8 项全过 ⑨ ruff 0 + mypy 0 + 全量 0 新失败 (W36b baseline) ⑩ CHANGELOG W36c 节点 + docs/MEMORY.md + docs/P5-RETRO.md |
| **W36d** | ① `pyproject.toml` version 升级 `0.5.0a1 → 0.5.0a2` ② `__version__` 同步升级 ③ `CHANGELOG.md` 新增 `0.5.0a2` 节点, 合并 W33a/W33b/W34/W35/W36a/W36b/W36c 全部 DoD/数据/已知限制 ④ `python -m build` 验证 sdist + wheel 构建成功 ⑤ `twine check dist/*` PASSED (W27 模式复用) ⑥ `git tag 0.5.0a2` 标签 ⑦ `tools/verify_w36d_dod.py` 8 项全过 (version 一致 / CHANGELOG 节点 / dist 文件存在 / twine check / tag 存在 / 全量回归) ⑧ ruff 0 + mypy 0 + 全量 1204+ passed (W36c baseline) ⑨ docs/MEMORY.md release 经验 + docs/P5-RETRO.md W36d 段 ⑩ 已知缺口 (TestPyPI 上传 / DESIGN untrack / 端到端) 状态更新 |
