...
| 优先级 | 候选 | 工作量 | 价值 | 备注 |
|-------|------|-------|------|------|
| **高** | **W33 WebState 持久化**（SQLite/Redis 切换,W29 下沉） | 1-2 周 | Web UI 重启不丢事件 | 与 W34 鉴权正交 |
| **高** | **W34 RBAC / auth v0**（JWT 签发 + 路由级权限,W30 下沉） | 2-3 周 | 多用户生产部署前置 | 飞书 webhook 已用 HMAC,需统一抽象 |
| **中** | **W35 Web UI v2**（agents/tasks 页面交互：从只读到操作） | 2 周 | 真正"可操作"而非只读仪表盘 | HTMX 已铺好,增量 |
| **中** | **W36 Golden Case G-022**（Web UI 端到端：webhook → swarm → WS 推送 → 浏览器可见） | 1 周 | 把 P5 锁进 CI 守门 | 复用 W15 Prometheus 思路 |
